...
function loadsTemplateFromLocalStorage() {
  ...
  try {
    currentTemplateIndex = parseInt(
      localStorage.getItem(LS_INDEX_ITEM_KEY) || "0",
      10,
    );
  } catch (e) {
    console.error(`Error loading templates from localStorage: ${e}`);
...
export default function LatexOnHttpDemoApp() {
  ...
  useEffect(() => {
    ...
    const [initialTemplates, initialCurrentTemplateIndex] =
  ...
  const convertToPdf = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Error LaTeX-on-HTTP compilation error:", jsonResp);
        setCompError(jsonResp);
      }
  ...
  return (
    <div id="app">
      ...
      <div>
        ...
        <button class="convert" onClick={convertToPdf} disabled={isCompiling}>
      ...
      {compError && (
        <div class="error sourceFull">
          <h4>{compError.error}</h4>
		  <CodeMirror
            value={compError.logs}
            extensions={[
              jsonLang({ base: jsonLanguage, codeLanguages: languages }),
            ]}
            readOnly
          />
        </div>
